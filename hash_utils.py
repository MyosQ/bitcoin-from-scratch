...
def gen_sha256_with_variable_scope_protector_to_not_pollute_global_namespace():
    ...
    def pad(b):
        ...
        le = len(b) * 8  # note: len returns number of bytes not bits
    ...
    def sha256(b: bytes) -> bytes:
        ...
        blocks = [b[i : i + 64] for i in range(0, len(b), 64)]  # noqa
        ...
        for M in blocks:  # each block is a 64-entry array of 8-bit bytes
            ...
            for t in range(64):
                if t <= 15:
                    ...
                    W.append(bytes(M[t * 4 : t * 4 + 4]))  # noqa
                else:
